...
use Carbon\Carbon;
use App\Exceptions\InvalidTimeStringException;
...
if (! function_exists('__user')) {
     ...
     *
     * @return \App\Administrator|\App\User|null
    ...
    function __user()
    {
        ...
        throw new \App\Exceptions\UndeterminedUserException('Could not determine current user');
    }
}
...
if (! function_exists('superAdmin')) {
    ...
    {
        return (bool) auth('admin')->user()->root_user;
    }
}
...
if (! function_exists('replace_between')) {
    /**
     * Replace a substring within given delimiters
     *
     * @param $beginning
     * @param $end
     * @param $string
     ...
     * @return string
     */
    function replace_between($beginning, $end, $string, $replacement): string
    {
        ...
        $strToReplace = substr($string, $beginningPos, ($endPos + strlen($end)) - $beginningPos);

        return str_replace($strToReplace, '', $replacement);
    }
}
